...
function showPage(name) {
    ...
    if (topbarAction)
        topbarAction.innerHTML =
            '<span class="ms" style="font-size:16px">add</span> ' +
            (info.action || "");
...
function updateWage(sel) {
    ...
    document.getElementById("total-val").textContent =
        (count * parseInt(wage)).toLocaleString("ar-EG") + " ج";
}
...
function addWorker() {
    ...
    document.getElementById("total-val").textContent =
        (workerCount * wage).toLocaleString("ar-EG") + " ج";
...
function openPayModal(company, amount) {
    ...
    document.getElementById("pay-amount").value =
        parseInt(amount).toLocaleString("ar-EG") + " ج";
    document.getElementById("pay-before").textContent =
        parseInt(amount).toLocaleString("ar-EG") + " ج";
...
function openCompanyModal(isEdit = false, companyId = null) {
    ...
    if (!form || !modal) {
        console.error(
            "Company form modal or form elements not found on this page",
        );
        return;
    }
    ...
    if (isEdit && companyId) {
        ...
        fetch(`/contractor/companies/${companyId}`, {
            ...
            .then((data) => {
                ...
                document.getElementById("form-contact_person").value =
                    data.contact_person;
                document.getElementById("form-phone").value = data.phone;
                document.getElementById("form-daily_wage").value =
                    data.daily_wage;
                document.getElementById("form-payment_cycle").value =
                    data.payment_cycle;
                document.getElementById("form-weekly_pay_day").value =
                    data.weekly_pay_day || "";
                document.getElementById("form-contract_start_date").value =
                    data.contract_start_date;
                document.getElementById("form-is_active").value = data.is_active
                    ? "1"
                    : "0";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (currentPath.includes("/companies")) {
        ...
        document
            .querySelectorAll(".mobile-footer-link")
